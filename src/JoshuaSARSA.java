 ...
 */
public class JoshuaSARSA implements IPlayer {

	private static final float LEARN_RATE = 0.00024f;
	private static final float DISCOUNT_FACTOR = 0.75f;
	private static final float EPSILON = 0.05f;
	private static final int NUM_FEATURES = 11;
	protected float[] Weights;
	private float PrevBoardQValue;
	...
	public int[] makeMove(IBoard board) {
		...
		// random action with probability EPSILON, otherwise greedy
		if ((float)Math.random() < EPSILON) {
			//random
			resultmove = movesList.get((int) (Math.random() * movesList.size()));
		}
		else {
			// get the move to do
			if (bestMovesList.size() == 1) {
				// could call this random too, I don't do it for performance reasons
				resultmove = bestMovesList.get(0);
			} else {
				// random for the case that the bestMovesListe is larger than 1 element
				resultmove = bestMovesList.get((int) (Math.random() * bestMovesList.size()));
			}
		}
		...
		if (board.getMoveHistory().size() > 2) {
			IBoard boardCopy = board.clone();
			try {
				boardCopy.makeMove(new Move(this, resultmove));
				updateWeights(PrevBoardQValue, getBoardValue(boardCopy), 0, PrevBoardFeatures);
			} catch (IllegalMoveException e) {
				// illegal move catching
			}
		}
		PrevBoardQValue = curBoardValue;
	...
	protected void updateWeights(float QValuePrev, float QValueNext, float reward, int[] boardFeaturesPrev) {
		
		float trainValue = reward + DISCOUNT_FACTOR*QValueNext - QValuePrev;
		trainValue *= LEARN_RATE;
		for (int i = 0; i < NUM_FEATURES; i++) {
			Weights[i] += trainValue * boardFeaturesPrev[i];
		}
	}
	...
	public void onMatchEnds(IBoard board) {
		if (board.getWinner() == null) {
			// draw
			updateWeights(0, 0, -25, getBoardFeatures(board));
			writeToFile(MyFileWinLoose, "0");
		} else if (board.getWinner() == this) {
			// win
			updateWeights(0, 0, 100, getBoardFeatures(board));
